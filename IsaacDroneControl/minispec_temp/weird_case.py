...
class VLMWrapper:
    ...
    def send_request(self, image: str = None, prompt: str = None, stream=False):
        ...
        """
        if prompt == None:
            prompt = "Is there something in the image that could be a vending machine in view? What motion could be taken to make it fully in view"
        try:
...
class LLMWrapper:
    ...
    def send_request(self, prompt: str, task: str, stream=False):
        """
        ...
        try:
            prompt += (
                ...
                f"[A] Move towards the vending machine with such directions: {task}\n\n"
...
def fetch_minispec_from_llm(short_prompt: str, prompt: str):
    """Fetch MiniSpec plan dynamically from the LLM."""
    print("Fetching MiniSpec plan from LLM...")
    try:
        # We can directly use the llm and vlm objects that were passed in
        vlm_response = vlm.send_request()
        llm_response = llm.send_request(prompt, task=vlm_response).strip()
        print(f"LLM's response: {llm_response}")
...
class SecondCodeHandler:
    """
    This class wraps all the logic that was originally in your
